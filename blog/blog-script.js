...
function displayPosts(posts) {
  ...
  if (posts.length === 0) {
    container.html(
      '<p class="text-center">No posts found matching the selected filters.</p>',
    );
    return;
  }
  ...
  posts.forEach((post) => {
    ...
    const thumbnailHtml = post.thumbnail
      ? `<img src="${post.thumbnail}" alt="${post.thumbnailCredit}" class="blog-post-thumbnail" onerror="this.style.display='none'">`
      : "";
